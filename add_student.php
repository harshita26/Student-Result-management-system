                ...
                <form action="add_student.php" method="post">
                    <fieldset>
                        ...
                        <div class="form-group">
                            <input type="text" class="form-control bg-secondary border-0 text-white"name="sname" placeholder="Student Name">
                        </div>
                        <div class="form-group">
...
if(isset($_POST['add'])){
    include('db.php');
    $class_name=$_POST['sname'];
    $class_id=$_POST['rollno'];
    $select_class=$_POST['dropdown'];
    $qry="INSERT INTO `add_student`( `sname`, `sroll`, `sclass`) VALUES ('$class_name','$class_id','$select_class')";
    $run=mysqli_query($con,$qry);    
    if(!$run){
        ?>
            <script>document.getElementById("demo").innerHTML="STUDENT IS NOT ADDED";</script>
        <?php
    }
    else{
        ?>
            <script>document.getElementById("demo").innerHTML="STUDENT IS ADD";</script>
        <?php
    }
}
?>
